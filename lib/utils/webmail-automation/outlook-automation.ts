/**
 * Implementação da automação de webmail para Outlook
 * Permite automação indetectável das operações no Outlook Web
 */
...
/**
 * Classe para automação específica do Outlook
 * Implementa funcionalidades específicas do Outlook usando a classe base
 */
export class OutlookAutomation extends BaseWebmailAutomation {
  ...
  // Selectors para elementos do Outlook Web
  private selectors: OutlookSelectors = {
    loginPage: {
      emailInput: 'input[type="email"]',
      emailNextButton: 'input[type="submit"]',
      passwordInput: 'input[type="password"]',
      passwordNextButton: 'input[type="submit"]',
      staySignedInCheckbox: 'input[name="KMSI"]',
      staySignedInButton: 'input[type="submit"]'
    },
    mailbox: {
      inboxSelector: 'div[title="Inbox"]',
      ...
      emailSubject: 'div[aria-label*="Subject"] span',
      emailBody: 'div[role="region"][aria-label="Message body"]',
      searchBox: 'input[aria-label="Search"]',
      logoutButton: 'a[data-task="signout"]',
      accountButton: 'button[aria-label*="Account manager"]',
      folderPane: 'div[role="tree"][aria-label="Folder pane"]'
    },
    compose: {
      toInput: 'div[aria-label="To"] input',
  ...
  constructor(options: {
    ...
    this.fingerprintGenerator = new BrowserFingerprintGenerator({
      // Outlook tem melhor compatibilidade com Edge e Chrome
      browserBase: Math.random() > 0.5 ? 'edge' : 'chrome'
    });
    this.humanBehavior = new HumanBehaviorSimulator();
  }

  /**
   * Inicializa o navegador para automação do Outlook
   */
  public async initialize(): Promise<boolean> {
    if (this.mockMode) {
      // Modo de simulação para testes
      this.state = 'ready';
      console.log('Iniciando Outlook Automation em modo de simulação');
      return true;
    }
    ...
    try {
      ...
      console.error('Erro ao inicializar navegador para Outlook:', error);
      this.state = 'error';
      return false;
    }
  }
  ...
  public async login(account: WebmailAccount): Promise<ActionResult> {
    ...
    try {
      // Navegar para a página de login do Outlook
      await this.page.goto('https://outlook.live.com/owa/', { waitUntil: 'networkidle' });
      
      // Clicar no botão de Sign In se estiver na página inicial
      try {
        const signInButton = await this.page.$('a[data-task="signin"]');
        if (signInButton) {
      ...
      // Clicar no botão de próximo para enviar senha
      await this.page.click(this.selectors.loginPage.passwordNextButton);
      
      // Verificar se aparece a opção "Stay signed in"
      try {
        await this.page.waitForSelector(this.selectors.loginPage.staySignedInCheckbox, { timeout: 5000 });
        
        // Marcar "Stay signed in" e continuar
        if (await this.page.$(this.selectors.loginPage.staySignedInCheckbox)) {
          // Aleatoriamente decidir marcar ou não o checkbox
          if (Math.random() > 0.5) {
            await this.page.click(this.selectors.loginPage.staySignedInCheckbox);
          }
          await this.randomDelay('short');
          await this.page.click(this.selectors.loginPage.staySignedInButton);
        }
      } catch (e) {
        // Se não aparecer, pode ser que já esteja sendo redirecionado
        console.log('Tela "Stay signed in" não encontrada, continuando...');
      }
      
      // Esperar a conclusão do login
      try {
        // Tentar localizar a caixa de entrada, que indica sucesso no login
        await this.page.waitForSelector(this.selectors.mailbox.inboxSelector, { timeout: 20000 });
      } catch (error) {
        ...
        if (pageContent.includes('incorrect') || pageContent.includes('password is wrong')) {
          this.state = 'error';
      ...
      for (const cookie of cookies) {
        if (cookie.domain.includes('outlook.com') || cookie.domain.includes('live.com') || cookie.domain.includes('microsoft.com')) {
      ...
      console.error('Erro ao fazer login no Outlook:', error);
      this.state = 'error';
  ...
  public async logout(): Promise<ActionResult> {
    ...
    try {
      // Verificar se estamos na página principal do Outlook
      const currentUrl = this.page.url();
      if (!currentUrl.includes('outlook.live.com/') && !currentUrl.includes('outlook.office.com/')) {
        await this.page.goto('https://outlook.live.com/owa/', { waitUntil: 'networkidle' });
        await this.randomDelay('medium');
      }
      ...
      console.error('Erro ao fazer logout do Outlook:', error);
      this.state = 'error';
  ...
  /**
   * Executa uma ação específica no Outlook
   */
  public async executeAction(action: WebmailAction, params: Record<string, any> = {}): Promise<ActionResult> {
    ...
    try {
      ...
      console.error(`Erro ao executar ação '${action}' no Outlook:`, error);
      this.state = 'error';
  ...
  private async sendEmail(params: Record<string, any>): Promise<ActionResult> {
    ...
    try {
      ...
      await this.page.keyboard.press('Enter'); // Confirmar o email
      await this.randomDelay('medium');
      ...
      // Esperar feedback de envio (geralmente uma notificação)
      await this.page.waitForSelector('div[aria-live="polite"][role="alert"]', { timeout: 10000 });
      await this.randomDelay('medium');
  ...
  private async markEmailAs(params: Record<string, any>, action: 'read' | 'unread' | 'spam' | 'not-spam'): Promise<ActionResult> {
    try {
      ...
      if (params.index !== undefined) {
        ...
        // Selecionar o email clicando com botão direito
        emailElement = emails[params.index];
        await emailElement.click({ button: 'right' });
        found = true;
      } else if (params.subject) {
        ...
        // Selecionar o primeiro resultado com botão direito
      ...
      // Realizar a ação correspondente no menu de contexto
      let menuItemSelector = '';
      switch (action) {
        case 'read':
          menuItemSelector = 'button[aria-label="Mark as read"]';
          break;
          
        case 'unread':
          menuItemSelector = 'button[aria-label="Mark as unread"]';
          break;
          
        case 'spam':
          menuItemSelector = 'button[aria-label="Mark as junk"]';
          break;
          
        case 'not-spam':
          // Para não-spam, primeiro precisamos ir para a pasta de spam (junk)
          await this.page.click('div[title="Junk Email"]');
          await this.randomDelay('medium');
          ...
          const junkEmails = await this.page.$$(this.selectors.mailbox.emailListItems);
          if (junkEmails.length === 0) {
            return this.createActionResult(`mark-as-${action}`, false, 'Email não encontrado na pasta de spam');
          }
          
          await junkEmails[0].click({ button: 'right' });
          menuItemSelector = 'button[aria-label="Mark as not junk"]';
          break;
      }
  ...
  private async moveToFolder(params: Record<string, any>): Promise<ActionResult> {
    ...
    try {
      ...
      if (params.index !== undefined) {
        ...
        // Selecionar o email clicando com botão direito
        await emails[params.index].click({ button: 'right' });
        found = true;
      } else if (params.subject) {
        // Procurar pelo assunto
        await this.search({ query: params.subject });
        ...
        // Selecionar o primeiro resultado com botão direito
        await emails[0].click({ button: 'right' });
        found = true;
      }
      ...
      // Clicar na opção "Move"
      await this.page.click('button[aria-label="Move"]');
      await this.randomDelay('short');
      
      // Na lista de pastas, procurar a pasta de destino
      const folderSelector = `div[title="${params.folder}"]`;
      
      try {
        await this.page.waitForSelector(folderSelector, { timeout: 5000 });
        await this.page.click(folderSelector);
      } catch (error) {
        // Se a pasta não for encontrada, tentar expandir a árvore de pastas
        const moreFoldersSelector = 'div[title="More"]';
        await this.page.click(moreFoldersSelector);
        await this.randomDelay('short');
  ...
  private async createFolder(params: Record<string, any>): Promise<ActionResult> {
    ...
    try {
      // Clicar com botão direito na árvore de pastas
      await this.page.click(this.selectors.mailbox.folderPane, { button: 'right' });
      await this.randomDelay('short');
      ...
      await this.simulateHumanTyping(params.name, 'input[aria-label="Folder name"]');
      
      // Clicar em OK ou pressionar Enter
  ...
  private async search(params: Record<string, any>): Promise<ActionResult> {
    ...
    try {
      ...
      // Aguardar resultados (esperar pelo spinner de carregamento desaparecer)
